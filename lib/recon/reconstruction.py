...
def reconstruct_point_cloud(
    ...
    :param data_dir: directory to load reconstruction data
    :param img_dir: directory to load the stereo image pairs
    ...
    # load parameter
    K_1 = np.load(f"{data_dir}/K_1.npy")
    K_2 = np.load(f"{data_dir}/K_1.npy")
    ...
    for img_idx, name in enumerate(img_names):
        ...
        mask_1 = red_light_mask(img_1, line_1, gap_window=gap_window, sim=sim)
        mask_2 = red_light_mask(img_2, line_2, gap_window=gap_window, sim=sim)
        ...
        # triangulate points
        pts_recon = triangulate_points(pts_1.T, pts_2.T, K_1, K_2, T_W1, T_W2)
    ...
    print(f"\nFinished reconstruction of {img_dir}")
